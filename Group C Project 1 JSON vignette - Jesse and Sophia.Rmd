...
library(knitr)
library(tidyverse)
Raleigh_data <- Raleigh %>% select(city,country,averagetemperature,averagetemperatureuncertainty,Year,Month,Season,YearRange) %>% arrange(Year)

library(DT)
datatable(Raleigh_data, rownames=FALSE, colnames = c("City","Country","Average Temperature","Average Temperature (uncertainty)","Year","Month","Season","Year Range"))
```
...
g <- ggplot(Raleigh_2013, aes(Month,y=avgRaleigh1,fill=Season))
