...
deathRateData3 <- deathRateData %>% filter(Race != "All Races" | Sex != "Both Sexes")
...
# meanAdjustedDeathRateData %>% group_by(Race) %>% summarise(`Mean Life Expectancy`=RoundedMean(meanLifeExpectancy, 1), `Average Age-Adjusted Death Rate`=RoundedMean(meanMort, 1)) %>% kable(caption = "Table 4: Average life expectancy and age-adjusted death rate by race")
...
The line plots show a clear trend in both life expectancy and age-adjusted death rate by year. The first line plot shows that there is a steady increase in life expectancy as the year increases. The only time there is a significant drop in life expectancy from one year to the next is in 1918, and this can be attriuted to the influenza epidemic that killed many people. Conversely, there is a steady decline in age-adjusted death rates as the year increases, which makes sense because people are living longer, and less people are dying.
...
g1 + geom_boxplot() + geom_jitter(aes(color=Race)) +
     labs(title = "Life Expectancy by Race Boxplot", y = "Average Life Expectancy")

g2 <- ggplot(data = deathRateData, aes(x = Sex, y = AveLifeExp))
g2 + geom_boxplot() + geom_jitter(aes(color=Sex)) +
     labs(title = "Life Expectancy by Sex Boxplot", y = "Average Life Expectancy")


g3 <- ggplot(data = deathRateData, aes(x = Race, y = Mort))
g3 + geom_boxplot() + geom_jitter(aes(color=Race)) + 
     labs(title = "Age-Adjusted Death Rate by Race", y = "Age-Adjusted Death Rate per 100,000")

g4 <- ggplot(data = deathRateData, aes(x = Sex, y = Mort))
g4 + geom_boxplot() + geom_jitter(aes(color=Sex)) + 
     labs(title = "Age-Adjusted Death Rate by Sex", y = "Age-Adjusted Death Rate per 100,000")
```
...
######TRYING TO FIGURE OUT WHY OUR MEANS DONT MATCH FROM OUR TABLES AND GRAPHS###########

# a1 <- ggplot(meanAdjustedDeathRateData, aes(x = Sex, y = meanLifeExpectancy, fill = Sex))
# a1 + geom_bar(aes(fill = Sex, y = meanLifeExpectancy), stat = "identity", position = "dodge") + 
#      labs(title = "Bar Plot of Average Life Expectancy by Sex", y = "Average Life Expectancy") +
#      coord_cartesian(ylim=c(50,71))
# 
# a2 <- ggplot(meanAdjustedDeathRateData, aes(x = Race, y = meanLifeExpectancy, fill = Race))
# a2 + geom_bar(aes(fill = Race, y = meanLifeExpectancy), stat = "identity", position = "dodge") + 
#      labs(title = "Bar Plot of Average Life Expectancy by Race", y = "Average Life Expectancy") +
#      coord_cartesian(ylim=c(50,71))
#################
...
# meanCombinedData <- CombinedData %>% group_by(Race_Sex) %>% 
#                     summarise(avgDistLE = round(mean(DistFromMeanLE, na.rm = TRUE),1), 
#                               avgDistMort = round(mean(DistFromMeanMort), 1))
# 
# 
# j1 <- ggplot(data = meanCombinedData, aes(x = Race_Sex, y = avgDistLE))
# j1 + geom_bar(aes(fill = Race_Sex, y = avgDistLE), stat = "identity") +
#     labs(title = "Mean Distance from Average Life Expectancy by Race and Sex", x = "Race/Sex", 
#          y = "Mean Distance from Average Life Expectancy") +
#     scale_x_discrete(name = "Race/Sex Combinations", 
#                      labels = c("Female", "Male", "Black", "Black/F", "Black/M", "White",
#                               "White/F", "White/M")) +
#     scale_fill_discrete(name = "Race/Sex Combinations", 
#                         labels = c("Female", "Male", "Black", "Black/Female", "Black/Male", "White",
#                                 "White/Female", "White/Male"))
# 
# #Bar Plot of average distance from mean Mortality
# j2 <- ggplot(data = meanCombinedData, aes(x = Race_Sex, y = avgDistMort))
# j2 + geom_bar(aes(fill = Race_Sex, y = avgDistMort), stat = "identity") +
#     labs(title = "Mean Distance from Average Age-Adjusted Death Rate by Race and Sex", x = "Race/Sex", 
#          y = "Mean Distance from Average Age-Adjusted Death Rate") +
#     scale_x_discrete(name = "Race/Sex Combinations", 
#                      labels = c("Female", "Male", "Black", "Black/F", "Black/M", "White",
#                               "White/F", "White/M")) +
